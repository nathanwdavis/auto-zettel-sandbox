...
> **Progress, 2026-09-03 maintenance cycle (three of six moved, three left):**
> The cheapest piece is closed.
> [[time-horizon-dominates-rate--202701010013]] now carries a `source` link to
> [[housel-on-compounding-as-time-in-the-loop--202608311037]] and a paragraph
> that names both limits — Housel is a practitioner, not a proof, and the
> arithmetic half of the claim is definitional and needs no source. The link
> was added in both directions.
> Two of the three zettelkasten design claims gained the limiting paragraph
> this entry asked for rather than a citation:
> [[atomic-notes-compound-over-time--202701010001]] and
> [[a-slip-box-surprises-only-after-critical-mass--202609010113]] (which was
> not on this entry's list of six but makes the same kind of causal claim) now
> record that the experimental literature was searched and does not reach them
> — see [[no-experiment-in-this-base-tests-a-slip-box-claim--202609032215]].
> 202701010002 already carried its own such paragraph and gained a sentence.
> STILL OPEN, and deliberately: 202701010003 (titles stated as claims) has no
> qualification yet and nothing was found to ground it; and 202701010011 /
> 202701010012, the two arithmetic notes, were left exactly as they are. This
> cycle judged that the right move for those two is the one this entry
> suggested — say IN the note that they are definitional — but did not make it,
> because "this claim needs no source" is a decision about what counts as a
> claim in this base, and the same cycle had just spent its judgement on
> citation limits elsewhere. Next run, or a human, should just do it.
...
## 2026-09-03 — Tooling, HIGH: this environment class has neither the skill scripts nor pandoc, so no gate ran locally this cycle and two things were done by hand

- **status:** new        <!-- new | in-progress | answered | archived -->
- **priority:** high
- **asked_by:** human

Filed by the 2026-09-03 evidence cycle, about that cycle's own conduct rather
than about the notes.

WHAT WAS MISSING. The `zettel-bootstrap` skill directory in this session
contained only `SKILL.md` — no `scripts/`, no `references/`. So
`verify_refs.py`, `build_manifest.py`, `lint_citations.py`, `lint_links.py`,
`lint_skills.py` and `check_skill_sandbox.py` could not be run before the
push, and neither could `maintenance_run.sh`. `pandoc` is not installed
either. Session scope also forbids reading the skill repository
(`nathanwdavis/auto-zettel-skill`) from here, so fetching the scripts was not
an option and was not attempted. The CI workflow in `.github/workflows/gates.yml`
checks the skill repo out server-side and is what actually gated this cycle —
which is the design working as intended, but it means the gates ran AFTER the
push rather than before it.

WHAT WAS DONE BY HAND, both of which want checking by a run that has the tools:

(1) **Chicago strings were hand-written**, against the skill's explicit
instruction never to hand-write them. The two new records
[[urry-et-al-dont-ditch-the-laptop-just-yet--202609032200]] and
[[mueller-and-oppenheimer-the-pen-is-mightier-than-the-keyboard--202609032201]]
carry `citation_renderer: hand-rendered-pending-verify-refs` instead of
`pandoc`, so they are findable with one grep. The alternative was leaving the
strings empty, which risked failing the citation gate for a reason that had
nothing to do with the evidence. A run with the scripts should re-render both
and correct whatever differs; the `csl_json` on each is complete and is the
input that matters. Note for that run: the Urry record has 89 authors and
`csl_json` lists the first seven, so a renderer configured to demand the full
list will need the roster, which is captured verbatim in
`raw/202609032200-urry-et-al-dont-ditch-the-laptop-manuscript-excerpts.txt`.

(2) **manifest.json was regenerated by a reconstruction of build_manifest.py**,
not by build_manifest.py. The reconstruction was validated before it was
trusted: it was run against the tree at the previous commit and its output
compared byte-for-byte with the committed `manifest.json`, which matched
exactly. That pins down the schema it infers — notes from
permanent/literature/reference/moc sorted by key, each note's `links` sorted by
`target_id`, inquiry `result_notes` sorted, `note_count` counting notes and not
inquiries, `id_to_key` covering notes only, `json.dump(indent=2,
sort_keys=True, ensure_ascii=False)` plus a trailing newline. If CI's
`build_manifest.py --check` passes on this commit, the reconstruction is
confirmed on a second, larger sample and this half of the entry can be closed.
If it fails, the diff is the specification and the fix belongs in the notes,
not in the manifest.

WHAT WOULD CLOSE THIS. Either the skill scripts becoming available to this
environment class (the remote cycle path in `references/remote-execution.md`
presumably arranges that and did not here), or a local Mode-A run that
re-verifies the two records and re-renders their citations.

## 2026-09-03 — Source-access gaps and the next target, left open by the evidence cycle (inquiry 202609032123)

- **status:** new        <!-- new | in-progress | answered | archived -->
- **priority:** normal
- **asked_by:** human

Filed by the 2026-09-03 evidence cycle. Inquiry 202609032123 is answered, but
it named four candidate sources and only two entered the base. Here is what
happened to the rest, checked rather than assumed, all on 2026-09-03.

CLOSED AND WANTED, in rough order of value:

1. **Mueller and Oppenheimer 2014 full text** (Psychological Science 25.6,
   1159-68, DOI 10.1177/0956797614524581). Unpaywall `is_oa` false with no OA
   location; OpenAlex `oa_status` "closed" and `any_repository_has_fulltext`
   false, with only the SAGE landing page and a PubMed record; Semantic Scholar
   `openAccessPdf` status CLOSED. No green deposit exists. The base therefore
   holds its abstract and nothing else, and everything it states about the
   original study's numbers comes from the replicators' re-analysis of the
   original authors' own corrigendum data at https://osf.io/t43ua/. Those
   corrigendum files are themselves public and were NOT fetched this cycle —
   fetching them would upgrade the base from "hostile witness reports the
   numbers" to "the original authors' own deposited data", which is a real
   improvement and is probably the cheapest item on this list.
2. **Morehead, Dunlosky and Rawson 2019**, "How Much Mightier Is the Pen than
   the Keyboard for Note-Taking? A Replication and Extension of Mueller and
   Oppenheimer (2014)", Educational Psychology Review 31.3, 753-780, DOI
   10.1007/s10648-019-09468-2. Unpaywall `is_oa` false, Semantic Scholar
   CLOSED, and Crossref holds no abstract for it, so unlike the other two there
   is not even an abstract to capture. It is the other major independent
   replication and it is in this base only as rows in Urry's forest plots. It
   also tested an eWriter condition, which is the one design in this literature
   that separates the medium from the transcription behaviour.
3. **Roediger and Karpicke 2006**, "Test-Enhanced Learning", Psychological
   Science 17.3, DOI 10.1111/j.1467-9280.2006.01693.x — `is_oa` false.
4. **Sparrow, Liu and Wegner 2011**, "Google Effects on Memory", Science
   333.6043, DOI 10.1126/science.1207745 — `is_oa` false.

WHY 3 AND 4 ARE THE NEXT TARGET AND NOT MERELY MORE SOURCES. The finding of
this cycle is that the whole laptop-versus-longhand literature manipulates the
medium of capture and measures a quiz the same day, so it cannot reach a claim
about what happens to a note afterwards
([[no-experiment-in-this-base-tests-a-slip-box-claim--202609032215]]). Retrieval
practice and transactive memory are the two literatures that DO manipulate what
happens to material after it is recorded. That is where a test of a slip-box
claim could come from. It is also where a genuine contradiction may live, which
the inquiry that commissioned this work spotted first: if retrieval is what
consolidates learning, then delegating memory to paper has a measurable cost,
and Drexel's generation may have been right about something Chavigny dismissed
([[the-note-became-an-external-memory-only-when-memorizing-came-to-be-seen-as-a-cost--202609032115]]).
Look for the contradiction, not the corroboration.

THE RULE THAT MUST HOLD WHEN THAT RUN HAPPENS. Neither of those literatures is
about note taking, let alone slip-boxes. Capturing them on abstracts and then
writing a permanent note that says "retrieval practice supports/refutes the
slip-box" would be the exact stretch inquiry 202609032123 forbade and this
cycle refused. If the full texts are not reachable, the honest outcome is
another limiting note, or none.

A METHOD NOTE WORTH KEEPING, because it worked. The Urry full text was found
after Unpaywall and OpenAlex both reported the article closed. Semantic
Scholar's `openAccessPdf` field knew about a green deposit neither of them
listed; its URL (`psyarxiv.com/vqyw6/download`) returns only a JavaScript
shell, and the actual PDF came from the OSF API —
`api.osf.io/v2/preprints/<id>/` to `relationships.primary_file` to
`api.osf.io/v2/files/<id>/` to `links.download`. Worth adding to the
source-access ladder in `skills/source-access-triage`: Semantic Scholar is a
